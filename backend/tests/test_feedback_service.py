...
def test_save_feedback_happy_path_seeds_exemplar_with_description(db):
    ...
    try:
        ...
        # old category label.
        assert item.description in classifier._FEEDBACK_EXEMPLARS["Professional & Consulting Services"]
    finally:
        classifier._FEEDBACK_EXEMPLARS.clear()
        classifier._FEEDBACK_EXEMPLARS.update(original)


def test_save_feedback_rejects_document_owned_by_another_user(db):
    owner = _make_user(db, "owner2@test.com")
    attacker = _make_user(db, "attacker2@test.com")
    doc, item = _make_document_with_item(db, owner)

    with pytest.raises(ValueError, match="not found"):
        save_feedback(
            db=db, user_id=attacker.id, document_id=doc.id, line_item_id=item.id,
            corrected_category="Professional & Consulting Services",
        )
